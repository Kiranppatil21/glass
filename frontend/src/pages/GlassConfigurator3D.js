...
import { AdvancedDynamicTexture, TextBlock, Control, Line } from '@babylonjs/gui';
...
import { 
  ...
  CornerDownRight, PenTool, Maximize2, RotateCcw as Rotate3D, Pencil
} from 'lucide-react';
...
const GlassConfigurator3D = () => {
  ...
  const fetchPricingConfig = async () => {
    try {
      const response = await fetch(`${API_URL}/api/erp/glass-config/pricing`);
  ...
  const calculateAllPrices = async () => {
    ...
    try {
      ...
      const updatedItems = await Promise.all(glassItems.map(async (item) => {
        const response = await fetch(`${API_URL}/api/erp/glass-config/calculate-price`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-1 space-y-3">
            ...
            <div className="space-y-2">
              <Button className="w-full h-10 bg-gray-800 hover:bg-gray-900 text-white font-medium rounded-xl text-sm">
                GENERATE QUOTATION
              </Button>
              <Button className="w-full h-10 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl flex items-center justify-center gap-2 text-sm">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button 
                      ...
                      title={isLargePreview ? "Normal Size" : "Large Preview (for printing)"}
                    >
              ...
              <div className="relative">
                <canvas
                  ref={canvasRef}
                  className={`w-full ${isLargePreview ? 'h-[650px]' : 'h-[450px]'} transition-all duration-300 ${viewMode === VIEW_MODES.HIGH_CONTRAST ? 'bg-white' : ''}`}
            ...
            <div className="mt-3 bg-white rounded-xl shadow-sm p-3">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-xs text-gray-500">Order Total ({glassItems.length} item{glassItems.length > 1 ? 's' : ''})</p>
                  <p className="text-2xl font-bold text-gray-800">
                    {calculatingPrice ? <Loader2 className="w-5 h-5 animate-spin" /> : `₹${orderTotal.toLocaleString()}`}
                  </p>
                  <p className="text-[10px] text-gray-400">Including GST @ {pricingConfig?.gst_rate || 18}%</p>
                </div>
